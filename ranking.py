...
def _bayesian_rating(rating: float, review_count: int, prior: float, confidence: float) -> float:
    ...
    למה prior הוא קבוע (config.RATING_PRIOR) ולא ממוצע דינמי של קבוצת התוצאות
    הנוכחית (כפי שהנוסחה "הטהורה" של IMDb לכאורה מרמזת): בדקנו את זה בפועל מול
    בדיוק המקרה "2 ביקורות של 10.0 מול 10,000 ביקורות של 8.5" - וממוצע דינמי
    נכשל, גם בקבוצת תוצאות ריאליסטית של ~30 נכסים! הסיבה: אותו הנכס עם 2
    הביקורות שאנחנו מנסים "לחשוד" בו הוא בעצמו חלק מהממוצע שמחושב, אז הוא
    "מושך" את הממוצע כלפי מעלה ומחליש את התיקון על עצמו. פתרון תקין דורש
    עוגן יציב בלתי-תלוי בקבוצה הנוכחית - זה בדיוק מה ש-IMDb עושה בפועל
    (הממוצע שלהם מחושב על פני מיליוני סרטים היסטוריים, לא על פני קבוצת
    ההשוואה הנוכחית). כאן אין לנו "מאגר היסטורי" כזה, אז prior קבוע וסביר
    (8.0 מתוך 10) הוא הקירוב הטוב ביותר לזה.
    """
